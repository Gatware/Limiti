...
if (qp!=EEPROM.read(10)) {EEPROM.update(10,qp); lcd.setCursor(9,1); lcd.print("SET"); delay(1200);}
delay(200); lcd.clear();                    // Se è cambiata, memorizza l'impostazione in EEPROM 9.

// -------------------------------- Impostazione della capacità della batteria --------------------------------
lcd.setCursor(0,0); lcd.print("Vel. scar. batt.");
while(digitalRead(10)==LOW); // Attende che venga lasciato il pulsante
while(digitalRead(10)==HIGH) // Continua a leggere l'encoder finché non premo
  {
  // encoder();  (ora è chiamata dagli interrupt)
  ...
  if(VSB<20)VSB=20;
  if(VSB>50)VSB=50;
  lcd.setCursor(0,1); lcd.print(String(VSB)+"mV/h AutMx:"+(int(1000/VSB))+"h");
  };
if(B>1) Bip(); // Ho premuto il pulsante, perciò prosegue 
if (VSB!=EEPROM.read(14)) {EEPROM.update(14,VSB); lcd.setCursor(8,1); lcd.print("SET     "); delay(1200);}
delay(200); lcd.clear();                    // Se è cambiata, memorizza l'impostazione in EEPROM 9.

// -------------------------------- Impostazione dell'indicatore di direzione --------------------------------
lcd.setCursor(0,0); lcd.print("Indic. di direz.");
while(digitalRead(10)==LOW); // Attende che venga lasciato il pulsante
while(digitalRead(10)==HIGH) // Continua a leggere l'encoder finché non premo
  {
  // encoder();  (ora è chiamata dagli interrupt)
  ...
  lcd.setCursor(0,1);
  if(Bussola==0) lcd.print("Nord in alto");
  else           lcd.print("Bussola     ");
  };
if(B>1) Bip(); // Ho premuto il pulsante, perciò prosegue 
if (Bussola!=EEPROM.read(15)) {EEPROM.update(15,Bussola); lcd.setCursor(13,1); lcd.print("SET"); delay(1200);}
