...
if (checkGPS())
  ...
  if(fix.valid.status && (fix.status >= gps_fix::STATUS_STD) && fix.valid.speed)
    ...
    if(vm>100)
      {Fermo=0; xLCD=13; Direzione();} // Se la velocità è almeno 1km/h*, aggiorna lo stato della freccia.
    else if(Fermo==0)                  // *: Serve per evitare che da fermo la freccia giri casualmente.
      {lcd.setCursor(13,0); lcd.print(' '); Fermo=1;} // Se si è appena fermato, cancella la freccia, se lo ricorda e non lo fa più.
    }
    ...
    if(fix.valid.status)
      {
      if(fix.status == gps_fix::STATUS_NONE) lcd.write(byte(0)); // RTC interno
      else if(fix.status == gps_fix::STATUS_TIME_ONLY) lcd.print('t'); // Riesce a leggere ora e data
      else lcd.write(byte(0)); // Vede alcuni satelliti
      }
