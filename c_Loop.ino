...
if (checkGPS())
  ...
  if(fix.valid.status && (fix.status >= gps_fix::STATUS_STD) && fix.valid.speed)
    ...
    if(fix.valid.status)
      {
      if(fix.status == gps_fix::STATUS_NONE) timeStatus='-'; // RTC interno
      else if(fix.status == gps_fix::STATUS_TIME_ONLY) timeStatus='t'; // Legge ora e data
      else timeStatus='M'; // Vede alcuni satelliti
      }
