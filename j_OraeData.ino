...
void OraeData()
{
  if (checkGPS())
    ...
    if (fix.valid.altitude)
      ...
      if (alt < 10) lcd.print( ' ' );
      lcd.print( alt );
      if (alt < 100) lcd.print( ' ' );
      }
    else
      {
      lcd.setCursor(15,0); lcd.write(byte(0)); // NoFix
      lcd.setCursor(11,1); lcd.print(F(" -- ")); // Cancella l'altitudine
      }
    lcd.print( 'm' );
    ...
    if(fix.valid.date && fix.valid.time)
      ...
      if (fix.dateTime.minutes < 10) lcd.print( ' ' );
      lcd.print(fix.dateTime.minutes);
      lcd.print(':');
      
      if (fix.dateTime.seconds < 10) lcd.print( ' ' );
      lcd.print(fix.dateTime.seconds);

      // Data
      lcd.setCursor(0,1);
      if (fix.dateTime.date < 10) lcd.print( ' ' );
      lcd.print(fix.dateTime.date);
      lcd.print( '/' );
      
      if (fix.dateTime.month < 10) lcd.print( ' ' );
      lcd.print(fix.dateTime.month);
      lcd.print( '/' );  
          
      if (fix.dateTime.year < 10) lcd.print( '0' );
