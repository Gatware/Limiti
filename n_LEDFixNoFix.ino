void LEDFixNoFix()
{
if (!fix.valid.status || (fix.status < gps_fix::STATUS_STD)) // Se non c'è FIX:
  {
  F=0; Voff(); // Sicuramente il verde va spento
  if (!fix.valid.satellites || (fix.satellites == 0))
    { // Se i satelliti sono 0, fa oscillare il LED Rosso.
    Gosc=0;
    Rosc=1;
    }
  else
    {  // Se ce n'è almeno 1, fa oscillare il LED Giallo.
